...
void ShellButtons::CreateVectorFreeButtons()
{
    for (int i = 0; i < this->MAX_MATRIX_SIZE; ++i)
    {
        ...
        for (int j = 0; j < this->MAX_MATRIX_SIZE + 1; ++j)
        {
            ...
            this->vector_free_buttons[i][j]->text->setCharacterSize(50);
            this->vector_free_buttons[i][j]->text->setStyle(sf::Text::Italic);
            // this->vector_free_buttons[i][j]->text->setColor(sf::Color(0, 49, 83));
            this->vector_free_buttons[i][j]->text->setPosition(this->vector_poz->x + 10, this->vector_poz->y);
...
void ShellButtons::TextPut(sf::RenderWindow &window, sf::Event &event)
{
    ...
    else if (event.type == sf::Event::TextEntered and !sf::Keyboard::isKeyPressed(sf::Keyboard::Backspace) and this->string_Text.length() < 7)
    {
...
void ShellButtons::MouseAnimateButtons(const sf::Vector2f &mouse_position, sf::RenderWindow &window)
{
    if (!this->TextBool)
    {
        ...
        {
            if (vector_buttons[i]->getSprite().getGlobalBounds().contains(mouse_position) and vector_buttons[i]->BTN_WAIT)
            {
                if (i < 7)
